...
var card_to_take =
    window_width < 1400
        ? window_width < 1020
            ...
            : 3
        : 4;
var instructor_webinar_page = 0;
var instructor_webinar_total_page = 0;
var instructor_webinar_slide = true;

jQuery(document).ready(function () {
    renderInstructorWebinars({
        page: instructor_webinar_page,
        break: card_to_break,
        take: card_to_take,
        id: "instructor-webinars",
        slide: instructor_webinar_slide,
    });

    $(`#instructor-webinars-carousel-control-prev`).click(function () {
        instructor_webinar_slide = true;
        instructor_webinar_page -= 1;

        if (instructor_webinar_total_page > 1) {
            $(`#instructor-webinars-carousel-control-next`).show();
        }

        if (instructor_webinar_page == 0) {
            $(`#instructor-webinars-carousel-control-prev`).hide();
        }
    ...
    $(`#instructor-webinars-carousel-control-next`).click(function () {
        instructor_webinar_slide = false;
        instructor_webinar_page += 1;

        $(`#instructor-webinars-carousel-control-prev`).show();

        if (instructor_webinar_page == instructor_webinar_total_page - 1) {
            $(`#instructor-webinars-carousel-control-next`).hide();
        }
...
function renderInstructorWebinars(carousel) {
    $.ajax({
        url: "/api/instructor/webinars",
        data: {
        ...
        success: function (response) {
            if (response.hasOwnProperty("data")) {
                ...
                instructor_webinar_total_page = totalPages;
