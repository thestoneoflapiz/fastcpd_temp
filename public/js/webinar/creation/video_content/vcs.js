...
function get_sections() {
    $.ajax({
        url: "/webinar/management/content/sections",
        success: function (response) {
            var data = response.data;
            webinar_event = response.event;
    ...
    if (sections.length > 0) {
        sections.forEach((section, index) => {
            ...
            tablist.append(
                ...
                section.section_number
                }" role="tab">`+(webinar_event=="day"?"Daily Outline" : `Day ${index+1}`)+`</a></li>`
            );
...
function generate_section_content(is_new, section) {
    ...
    if (is_new == false) {
        ...
        if (section.detailed_sequence.length > 0) {
            ...
            // PartLectureUppy.init();
            FormDesignLecture.init();
    ...
    sequence.forEach((seq, key) => {
        ...
        .keyup(function (event) {
            ...
            if (value && value != "") {
                ...
                $(`#part-video-label-${data.section}-${data.part}`).html(`Lecture Title`);
            }
        });
        ...
        switch (seq.type) {
            case `video`:
                ...
                partcontent.find(`#part-video-title`).html(`Lecture — ${part_content_title_limit} &nbsp; <i class="fa fa-check-circle kt-font-success" style="font-size:1rem;"></i>`);
                break;
...
$(`#part-removal-modal-submit`).click(function (element) {
    ...
    if (current.exist == "true") {
        if (section.find(`#part-${current.section}-${current.part}`).remove()) {
            $.ajax({
                url: "/webinar/management/content/remove/part",
                data: {
                    type: current.type,
...
function uploadSummernoteImage(file, summernote_element) {
    ...
    $.ajax({
        url: "/webinar/management/content/textarea/upload",
        method: "POST",
        data: data,
        cache: false,
        contentType: false,
    ...
    var input_validations = function () {
        ...
        validator = $(`#section-form-${current_section}`).validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    url: "/webinar/management/content/section/store",
                    type: "POST",
                    data: {
                        objective: $(
...
var FormDesignLecture = (function () {
    var input_validations = function () {
        var data = {
        ...
        validator = $(`#part-video-form-${data.section}-${data.part}`).validate({
            ...
            submitHandler: function (form) {
                ...
                form_submit.addClass("kt-spinner kt-spinner--right kt-spinner--md kt-spinner--light disabled").html("Saving Lecture Title...").prop("disabled", true);
                var $video_id = $(`#video-id-${data.section}-${data.part}`).val();

                $.ajax({
                    url: "/webinar/management/content/video/store",
                    type: "POST",
                    data: {
                        video_id: $video_id,
                        title: $(`#part-video-input-${data.section}-${data.part}`).val(),
                    ...
                    success: function (response) { 
                        alert.addClass("kt-hidden").hide();
                        form_submit.removeClass("kt-spinner kt-spinner--right kt-spinner--md kt-spinner--light disabled").html("Save Lecture Title").prop("disabled", false);
    ...
    var input_validations = function () {
        ...
        validator = $(
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    url: "/webinar/management/content/article/store",
                    type: "POST",
                    data: {
                        article_id: $(
                            `#part-article-id-${data.section}-${data.part}`
                    ...
                    success: function (response) {
                        ...
                        if (response.status == 200) {
                            ...
                            partcontent
                                .find(`#part-article-title`)
                                .html(`Article — ${part_content_title_limit} &nbsp; <i class="fa fa-check-circle kt-font-success" style="font-size:1rem;"></i>`);
    ...
    var input_validations = function () {
        ...
        validator = $(`#part-quiz-form-${data.section}-${data.part}`).validate({
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    url: "/webinar/management/content/quiz/store",
                    type: "POST",
                    data: {
                        quiz_id: $(
                            `#part-quiz-id-${data.section}-${data.part}`
    ...
    var input_validations = function () {
        ...
        validator = $(
            ...
            submitHandler: function (form) {
                ...
                $.ajax({
                    url: "/webinar/management/content/quiz/item/store",
                    type: "POST",
                    data: {
                        quiz_id: $(
                            `#part-quiz-id-${data.section}-${data.part}`
