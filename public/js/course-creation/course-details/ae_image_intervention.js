...
var UppyImageIntervention = function () {
    ...
    var uppyCoursePoster = function () {
        ...
        uppyMin.use(XHRUpload, {
            endpoint: '/course/management/poster/upload', 
            method: 'post',
            formData: true,
            fieldName: 'files',
            metaFields: null,
        ...
        $(id + ' .uppy-FileInput-input').addClass('kt-uppy__input-control').attr('id', 'uppy_course_poster_input_control');
        var course_poster = $("input[name='course_poster']");
        if (course_poster.val() != "") {
            ...
            $(id + ' .uppy-FileInput-container').append('<label class="kt-uppy__input-label btn btn-label-brand btn-bold btn-font-sm attach-file-poster" for="uppy_course_poster_input_control">Attach file</label>');
        }
        ...
        uppyMin.on('complete', function (file) {
            $.each(file.successful, function (index, value) {
                ...
                $(`#course_poster_acc`).find(`div.fastcpd-background`).css("background-image", `url('${value.response.body.pathname}')`);
                $(`input[name="course_poster"]`).val(value.response.body.pathname);
...
function removeCoursePoster() {
    ...
    $.ajax({
        url: "/course/management/poster/remove",
        success: function () {
