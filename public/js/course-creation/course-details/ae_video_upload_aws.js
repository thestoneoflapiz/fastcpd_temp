...
function removeCourseVideo() {
    ...
    $.ajax({
        url: "/course/management/attract/video/remove",
        data: {
        ...
        success: function () {
            ...
            `);
            $(`#course_pv_acc_title`).empty().append(`<i class="fa fa-video circle-icon"></i> Promotional Video`);
...
function upload_video_to_s3(){
    ...
    if (files) {
        ...
        if(file.size > 25000000){ //25 MB since there's some file that are 19MB on windows os but 20MB more in system reading
        ...
        var fileName = `course_${$digest_[1]}${$digest_[2]}.${fileExtension}`;
...
function save_video_file(data){
    ...
    $.ajax({
        method: "POST",
        url: "/course/management/attract/video/upload",
        data: data,
        success: function () {
            ...
            `);
            $(`#course_pv_acc_title`).empty().append(`
