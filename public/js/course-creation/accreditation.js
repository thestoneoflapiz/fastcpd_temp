...
jQuery(document).ready(function () {
    ...
    $( "#print_app_form" ).click(function() {
        id = $("#course_id").val();
        window.open("/data/pdf/user/application_form/"+id);
    });
    $( "#view_cpdas" ).click(function() {
        id = $("#course_id").val();
        window.open("/course_management/cpdas/"+id);
    });
    ...
    var input_validations = function() {
        validator = $("#price_target_students_form").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: '/course_management/submit_for_accreditation/store',
                    type: 'POST',
                    data: {
                        price: $("#price").val(),
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            ...
                            setTimeout(() => {
                                window.location = "/course/management/submit_accreditation";
                            }, 1000);
                        } else {
    ...
    var inst_input_validations = function() {
        validator = $("#instructional_form").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: '/course_management/instructional_design/store',
                    type: 'POST',
                    data: {
                    ...
                    success: function(response) {
                        if (response.status == 200) {
                            ...
                            // setTimeout(() => {
                            //     window.location = "/data/pdf/user/application_form/"+response.id;
                            // }, 1000);
                        } else {
                            submit_button.html(`Submit for Accreditation`).removeClass('kt-spinner kt-spinner--right kt-spinner--md kt-spinner--light disabled').prop("disabled", false);
                            toastr.error('Error!', response.message);
    ...
    var expns_input_validations = function() {
        validator = $("#expendses_breakdown_form").validate({
            ...
            submitHandler: function(form) {
                ...
                $.ajax({
                    url: '/course_management/expenses_breakdown/store',
                    type: 'POST',
                    data: {
...
function can_submit(){
    $.ajax({
        url: "/course/management/is/prc-acc",
        success: function(response){
            if(response.allow){
                $(`#form`).show();
