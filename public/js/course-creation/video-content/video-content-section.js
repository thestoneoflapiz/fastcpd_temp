...
function generateExistingParts(section) {
    ...
    sequence.forEach((seq, key) => {
        ...
        partcontent
            .find(`#hidden-part-video-input`)
            .attr(`id`, `part-video-input-${data.section}-${data.part}`)
            .keyup(function (event) {
                var value = event.target.value;
                if (value && value != "") {
                    $(`#part-video-label-${data.section}-${data.part}`).html(
                        value
                    );
                } else {
                    $(`#part-video-label-${data.section}-${data.part}`).html(
        ...
        partcontent.find(`#hidden-part-video-upload-muted`).attr(`id`, `part-video-upload-muted-${data.section}-${data.part}`);
            
        partcontent
            .find(`#hidden-part-video-label`)
            .attr(`id`, `part-video-label-${data.section}-${data.part}`)
            .click(function () {
                $(this).hide();
        ...
        // Video Part Uppy
        partcontent
            .find(`#hidden-part-video-uppy`)
            .attr(`id`, `part-video-uppy-${data.section}-${data.part}`);
        partcontent
            .find(`#hidden-part-video-exist-buttons-div`)
            .attr(
                `id`,
                `part-video-exist-buttons-div-${data.section}-${data.part}`
            );
        partcontent
            .find(`#hidden-part-video-uploading-buttons-div`)
            .attr(
                `id`,
                `part-video-uploading-buttons-div-${data.section}-${data.part}`
            );    
        partcontent.find(`#hidden-part-video-refresh-button`).attr(`id`, `part-video-refresh-button-${data.section}-${data.part}`);
        partcontent.find(`#hidden-part-video-cancel-button`).attr(`id`, `part-video-cancel-button-${data.section}-${data.part}`);

        partcontent
            .find(`#hidden-part-video-view-button`)
            .attr(`id`, `part-video-view-button-${data.section}-${data.part}`);
        partcontent
            .find(`#hidden-part-video-remove-button`)
            .attr(
                `id`,
                `part-video-remove-button-${data.section}-${data.part}`
            );
        partcontent
            .find(`#hidden-video-url`)
            .attr(`id`, `video-url-${data.section}-${data.part}`);
        partcontent
            .find(`#hidden-video-filename`)
            .attr(`id`, `video-filename-${data.section}-${data.part}`);
        partcontent
            .find(`#hidden-video-size`)
            .attr(`id`, `video-size-${data.section}-${data.part}`);
        partcontent
            .find(`#hidden-video-length`)
            .attr(`id`, `video-length-${data.section}-${data.part}`);
        partcontent
            .find(`#hidden-video-poster`)
            .attr(`id`, `video-poster-${data.section}-${data.part}`);
        partcontent
            .find(`#hidden-video-id`)
            .attr(`id`, `video-id-${data.section}-${data.part}`);
        // Video Part Submit
        partcontent
            .find(`#hidden-part-video-form-submit`)
            .attr(`id`, `part-video-form-submit-${data.section}-${data.part}`);
        ...
        switch (seq.type) {
            case `video`:
                ...
                $(`#part-video-card-title-${data.section}-${data.part}`)
                    .attr(`aria-expanded`, false)
                    .addClass(`collapsed`);

                partcontent.find(`.nav-link.section_links`).hide();
                $(`#part-video-content-${data.section}-${data.part}`).addClass(
                    "active"
                );
                ...
                $(`#part-video-input-${data.section}-${data.part}`).val(
                    seq.title
                );
                /** video_id */
                ...
                var part_content_title_limit =
                    video_content_title.length > 35
                        ? `${video_content_title.substring(0, 35)}...`
                        : video_content_title;

                $(`#part-video-remove-button-${data.section}-${data.part}`).click(function () {
                    var remove_modal = $(`#part-removal-video-modal`);
                    remove_modal
                        .find(`#part-removal-video-modal-submit`)
                        .attr(`data-part`, data.part)
                        .attr(`data-section`, data.section);
                    remove_modal
                        .find(`.modal-body`)
                        .html(
                            `You're removing the <b>Part #${data.part}</b>'s video titled <b>"${seq.title}"</b> from your current <b>Section</b>.<br><br>Are you sure you want to remove it?`
                        );
                ...
                if(seq.cdn_url){
                    ...
                    partcontent.find(`.video-filename`).each(function (){
                        $(this).html(`Name: ${seq.filename}`);
                    });
                    partcontent.find(`.video-length`).html(`Length: ${seq["length"]} mins`);

                    $(`#part-video-uppy-${data.section}-${data.part}`).hide();
                    ...
                    if(seq.uploading_status=="uploading"){
                        ...
                        if(seq.uploading_status=="failed"){
                            ...
                            $(`#part-video-upload-muted-${data.section}-${data.part}`).html(`Your video has failed to upload.`).show();
                        }
                        $(`#part-video-uppy-${data.section}-${data.part}`).show();
                    }
                }
        ...
        $(`#part-quiz-body-textarea-${data.section}-${data.part}`).summernote({
            height: 80,
            toolbar: toolbar_show,
            callbacks: {
                onImageUpload: function (files, editor, welEditable) {
...
function generateParts(data) {
    ...
    part.find(`#hidden-part-video-input`)
        .attr(`id`, `part-video-input-${data.section}-${data.part}`)
        .keyup(function (event) {
            var value = event.target.value;
            if (value && value != "") {
    ...
    part.find(`#hidden-part-video-upload-muted`).attr(`id`, `part-video-upload-muted-${data.section}-${data.part}`);

    part.find(`#hidden-part-video-label`)
        .attr(`id`, `part-video-label-${data.section}-${data.part}`)
        .click(function () {
            $(this).hide();
            $(`#part-video-input-${data.section}-${data.part}`).show();
    ...
    // Video Part Uppy
    part.find(`#hidden-part-video-uppy`).attr(
        `id`,
        `part-video-uppy-${data.section}-${data.part}`
    );
    part.find(`#hidden-part-video-exist-buttons-div`).attr(
        `id`,
        `part-video-exist-buttons-div-${data.section}-${data.part}`
    );
    part.find(`#hidden-part-video-uploading-buttons-div`).attr(
        `id`,
        `part-video-uploading-buttons-div-${data.section}-${data.part}`
    );
    part.find(`#hidden-part-video-refresh-button`).attr(`id`, `part-video-refresh-button-${data.section}-${data.part}`);
    part.find(`#hidden-part-video-cancel-button`).attr(`id`, `part-video-cancel-button-${data.section}-${data.part}`);
    part.find(`#hidden-part-video-view-button`).attr(
        `id`,
        `part-video-view-button-${data.section}-${data.part}`
    );
    part.find(`#hidden-part-video-remove-button`).attr(
        `id`,
        `part-video-remove-button-${data.section}-${data.part}`
    );
    part.find(`#hidden-video-url`).attr(
        `id`,
        `video-url-${data.section}-${data.part}`
    );
    part.find(`#hidden-video-filename`).attr(
        `id`,
        `video-filename-${data.section}-${data.part}`
    );
    part.find(`#hidden-video-size`).attr(
        `id`,
        `video-size-${data.section}-${data.part}`
    );
    part.find(`#hidden-video-length`).attr(
        `id`,
        `video-length-${data.section}-${data.part}`
    );
    part.find(`#hidden-video-poster`).attr(
        `id`,
        `video-poster-${data.section}-${data.part}`
    );
    part.find(`#hidden-video-id`).attr(
        `id`,
        `video-id-${data.section}-${data.part}`
    );
    // Video Part Submit
    part.find(`#hidden-part-video-form-submit`).attr(
        `id`,
        `part-video-form-submit-${data.section}-${data.part}`
    );
...
function generate_quiz_item_content(item, data) {
    ...
    $(`#part-quiz-body-textarea-${data.section}-${data.part}`).summernote("code",item.question);
...
$(`#part-removal-video-modal-submit`).click(function (e) {
    ...
    $.ajax({
        ...
        success: function (response) {
            $(`#part-video-uppy-${data.section}-${data.part}`).show().find(`.kt-uppy__drag`).show();
...
function partVideoRefresh(data, $video){
    ...
    $.ajax({
        ...
        success: function (response) {
            $(`#part-video-uppy-${data.section}-${data.part}`).show().find(`.kt-uppy__drag`).hide();
            $(`#part-video-exist-buttons-div-${data.section}-${data.part}`).show();
            $(`#part-video-uploading-buttons-div-${data.section}-${data.part}`).hide();
...
var PartVideoUppy = (function () {
    ...
    var inituppy = function () {
        ...
        var id = `#part-video-uppy-${data.section}-${data.part}`;
        var video_id = $(`#video-id-${data.section}-${data.part}`);
        var video_title = $(`#part-video-input-${data.section}-${data.part}`);
        var $urlTo = `/course_management/upload/video/content?video_id=${video_id.val()}`;
        var form_video_sub_btn = $(`button#part-video-form-submit-${data.section}-${data.part}`);

        var uppyDrag = Uppy.Core({
            autoProceed: true,
            restrictions: {
                maxFileSize: 3000000000, // 2gb
                maxNumberOfFiles: 1,
                minNumberOfFiles: 1,
        ...
        if ($(`${id} .kt-uppy__drag button`).length) {
            ...
            $(id).on('DOMSubtreeModified', ".kt-uppy__progress .uppy-ProgressBar .uppy-ProgressBar-percentage", function() {
                var percentage_progress = $(id).find(".kt-uppy__progress .uppy-ProgressBar .uppy-ProgressBar-percentage");
                $(id).find(".kt-uppy__progress_percentage_display").html(`${percentage_progress.html()}%`);
            });
        }
        ...
        uppyDrag.on("upload", (data) => {
            $(id).find(`.kt-uppy__drag`).hide();
            form_video_sub_btn.addClass("disabled").prop("disabled", true);
            $(".kt-uppy__progress_percentage_display").show();
        });

        uppyDrag.on("complete", function (file) {
            form_video_sub_btn.removeClass("disabled").prop("disabled", false);
            $(".kt-uppy__progress_percentage_display").hide();
            ...
            $.each(file.successful, function (index, value) {
                ...
                imagePreview += `<div class="kt-uppy__thumbnail-container" data-id="${
                    value.id
                        ...
                        ${thumbnail}
                        <i class="fa fa-check-circle" style="color:#00b764;height:10px;"></i>
                        <span class="kt-uppy__thumbnail-label"> ${
                    value.name
                    } ( ${Math.round(filesize, 2)} ${sizeLabel})</span>
                        <span data-id="${
                    value.id
                    }" class="kt-uppy__remove-thumbnail">
                ...
                /**
                 * Missing
                 * video-length, video-poster, video-id
                 *
                 */
                var part = $(`#part-content-${data.section}-${data.part}`);
                $(`#part-video-view-button-${data.section}-${data.part}`).click(function () {
                    window.open(`/course/management/content/preview/${video_id.val()}`);
                });
                var partvideotitle = $(`#part-video-label-${data.section}-${data.part}`).text();
                var partvideotitle_limit = partvideotitle.length > 35
                ? `${partvideotitle.substring(0, 35)}...`
                : partvideotitle;

                if(value.response.body.upload_status == "done"){
                    part.find(`#part-video-title`).html(`Video — ${partvideotitle_limit} &nbsp; <i class="fa fa-check-circle kt-font-success" style="font-size:1rem;"></i>`);
                    $(`#part-video-exist-buttons-div-${data.section}-${data.part}`).show();
                    part.find(`.video-length`).html(`Length: ${value.response.body.duration} mins`);
                    $(`#part-video-remove-button-${data.section}-${data.part}`).click(function() {
                        var imageId = $(`${id} .kt-uppy__thumbnails .kt-uppy__remove-thumbnail`).attr("data-id");
                        uppyDrag.removeFile(imageId);
                        $(`${id} .kt-uppy__thumbnail-container[data-id="${imageId}"`).remove();

                    });
                }else{
                    part.find(`#part-video-title`).html(`Video — ${partvideotitle_limit} &nbsp; <i class="fa fa-minus-circle kt-font-warning" style="font-size:1rem;"></i>`);
                    $(`#part-video-uploading-buttons-div-${data.section}-${data.part}`).show();
                    $(`#part-video-refresh-button-${data.section}-${data.part}`).click(function(){
                        partVideoRefresh(data, {id:video_id.val(), title:video_title.val()});
                    });
                    $(`#part-video-cancel-button-${data.section}-${data.part}`).click(function(){
                        var imageId = $(`${id} .kt-uppy__thumbnails .kt-uppy__remove-thumbnail`).attr("data-id");
                        uppyDrag.removeFile(imageId);
                        ...
                        partVideoCancelUpload(data, {id:video_id.val(), title:video_title.val()});
                    });
                }
                
                $(`#part-video-uppy-${data.section}-${data.part}`).hide();
                part.find(`.video-filename`).each(function(){
                    $(this).html(`Name: ${value.response.body.filename}`);
                });

                $(`#video-url-${data.section}-${data.part}`).val(value.response.body.pathname);
                $(`#video-filename-${data.section}-${data.part}`).val(value.response.body.filename);
                $(`#video-size-${data.section}-${data.part}`).val(value.response.body.filesize);
            });
            ...
            if (file.failed) {
                    ...
                    imagePreview += `<div class="kt-uppy__thumbnail-container" data-id="${
                        value.id
                            ...
                            ${thumbnail}
                            <i class="fa fa-exclamation-circle" style="color:red;height:10px;"></i>
                            <span class="kt-uppy__thumbnail-label" style="color:red;">${
                        value.name
                        } (${Math.round(filesize, 2)} ${sizeLabel})</span>
                            <span data-id="${
                        value.id
        ...
        $(document).on(
            ...
            function () {
                $(id).find(`.kt-uppy__drag`).show();
                var imageId = $(this).attr("data-id");
                var part = $(`#part-content-${data.section}-${data.part}`);
                part.find(`.video-filename`).each(function(){
                    $(this).html(`Name: Unknown`);
                });
                part.find(`.video-length`).html(`Length: Unknown`);
                $(`#video-url-${data.section}-${data.part}`).val(null);
                $(`#video-filename-${data.section}-${data.part}`).val(null);
                $(`#video-size-${data.section}-${data.part}`).val(null);
                form_video_sub_btn.removeClass("disabled").prop("disabled", false);
        ...
        validator = $(`#part-video-form-${data.section}-${data.part}`).validate(
            {
                ...
                submitHandler: function (form) {
                    ...
                    form_submit
                        .addClass("kt-spinner kt-spinner--right kt-spinner--md kt-spinner--light disabled").html("Saving Video Title...").prop("disabled", true);
