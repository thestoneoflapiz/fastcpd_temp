...
var card_to_take = window_width < 1400 ? (window_width < 740 ? 1 : 2) : 3;

var provider_course_page = 0;
var provider_course_total_page = 0;
var provider_course_slide = true;

var provider_instructors_page = 0;
var provider_instructors_total_page = 0;
var provider_instructors_slide = true;
...
jQuery(document).ready(function () {
    ...
    renderProviderInstructors({
        page: provider_instructors_page,
        break: card_to_break,
        take: card_to_take,
        id: "provider-instructors",
        slide: provider_instructors_slide,
    });

    $(`#provider-instructors-carousel-control-prev`).click(function () {
        provider_instructors_slide = true;
        provider_instructors_page -= 1;

        if (provider_instructors_total_page > 1) {
            $(`#provider-instructors-carousel-control-next`).show();
        }

        if (provider_instructors_page == 0) {
            $(`#provider-instructors-carousel-control-prev`).hide();
        }
    ...
    $(`#provider-instructors-carousel-control-next`).click(function () {
        provider_instructors_slide = false;
        provider_instructors_page += 1;

        $(`#provider-instructors-carousel-control-prev`).show();

        if (provider_instructors_page == provider_instructors_total_page - 1) {
            $(`#provider-instructors-carousel-control-next`).hide();
        }
...
function renderProviderInstructors(carousel) {
    $.ajax({
        url: "/api/provider/instructors",
        data: {
            provider: $(`input[name="provider_id"]`).val(),
            page: carousel.page,
            take: carousel.take,
        ...
        success: function (response) {
            if (response.hasOwnProperty("data")) {
                ...
                var totalPages =
                    (num_loop < 1 ? 1 : num_loop) + (remainder ? 1 : 0);
                provider_instructors_total_page = totalPages;
...
function renderInstructorWithCarousel(response, carousel) {
    ...
    data.forEach((profile, index) => {
        ...
        <div class="kt-widget__body">
            ...
            <div class="kt-widget__item">
                <div class="kt-widget__contact">
                    <span  style="font-size:1rem;"><span class="kt-font-bolder kt-font-info" style="font-size:1.3rem;">${profile.total_courses.toLocaleString()}</span>&nbsp; &nbsp; Courses</span>
                    <span  style="font-size:1rem;"><span class="kt-font-bolder kt-font-info" style="font-size:1.3rem;">${profile.total_webinars.toLocaleString()}</span>&nbsp; &nbsp; Webinars</span>
                    <span  style="font-size:1rem;"><span class="kt-font-bolder kt-font-info" style="font-size:1.3rem;">${profile.total_providers.toLocaleString()}</span>&nbsp; &nbsp; Assoc. Providers</span>
                </div> <br/>
