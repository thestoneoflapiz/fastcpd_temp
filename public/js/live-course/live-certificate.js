jQuery(document).ready(function () {
    $(`.get_cpd_certificate_button`).click(function(){
        if(assessment.assessment==="true"){
            if(has_completed_quiz && has_completed_rating && has_completed_course){
                ...
                if(!has_quiz && has_completed_rating && has_completed_course){
                    toastr.info(`Congratulations! Your CPD certificate will generate!!!`);
                    setTimeout(function(){
                        get_certificate();
                    }, 2500);
                }else{
                    toastr.error(`Sorry! You haven't completed the requirements of this course yet. Kindly check again, or refresh your browser.`);
                }
            }
        }else{
            if(has_completed_rating && has_completed_course){
                toastr.info(`Congratulations! Your CPD certificate will generate!!!`);
                setTimeout(function(){
                    get_certificate();
                }, 2500);
...
function live_completed_course(params) {
    var span = $("#span_completed_course");
