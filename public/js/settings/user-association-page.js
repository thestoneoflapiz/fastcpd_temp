...
var course_page = 0;
var course_total_page = 0;
var course_slide = true;

var provider_page = 0;
var provider_total_page = 0;
var provider_slide = true;
...
jQuery(document).ready(function () {
    ...
    $(`#courses-card-carousel-control-prev`).click(function () {
        course_slide = true;
        course_page -= 1;

        if (course_total_page > 1) {
            $(`#courses-card-carousel-control-next`).show();
        }

        if (course_page == 0) {
            $(`#courses-card-carousel-control-prev`).hide();
        }
    ...
    $(`#courses-card-carousel-control-next`).click(function () {
        course_slide = false;
        course_page += 1;

        $(`#courses-card-carousel-control-prev`).show();

        if (course_page == course_total_page - 1) {
            $(`#courses-card-carousel-control-next`).hide();
        }
    ...
    renderProvider({
        ...
        take: card_to_take,
        id: "provider-card",
        slide: provider_slide,
    });

    $(`#provider-card-carousel-control-prev`).click(function () {
        provider_slide = true;
        provider_page -= 1;

        if (provider_total_page > 1) {
            $(`#provider-card-carousel-control-next`).show();
        }

        if (provider_page == 0) {
            $(`#provider-card-carousel-control-prev`).hide();
        }
    ...
    $(`#provider-card-carousel-control-next`).click(function () {
        provider_slide = false;
        provider_page += 1;

        $(`#provider-card-carousel-control-prev`).show();

        if (provider_page == provider_total_page - 1) {
            $(`#provider-card-carousel-control-next`).hide();
        }
...
function renderCourses(carousel) {
    $.ajax({
        url: "/api/user/associated/courses",
        data: {
            page: carousel.page,
            take: carousel.take,
        },
...
function renderProvider(carousel) {
    $.ajax({
        url: "/api/user/associated/providers",
        data: {
            page: carousel.page,
            take: carousel.take,
        },
        success: function (response) {
            if (response.hasOwnProperty("data")) {
                ...
                var totalPages =
                    (num_loop < 1 ? 1 : num_loop) + (remainder ? 1 : 0);
                provider_total_page = totalPages;
...
function renderProviderWithCarousel(response, carousel) {
    ...
    data.forEach((profile, index) => {
        ...
        <div class="kt-widget__footer">
            <button type="button" class="btn btn-label-info btn-lg btn-upper" onclick="window.location='/provider/session/${row.id}'">visit portal</button>
        </div>
    </div>`);
