...
<form class="kt-form kt-form--label-left" id="form">
    <div class="kt-portlet kt-portlet--mobile">
        ...
        <div class="kt-portlet__body">
                ...
                <div class="kt-portlet__body">
                    <div class="form-group row">
                        ...
                        <div class="col-xl-6 col-lg-6 col-md-6 col-sm-12">
                            <select class="form-control kt-select2" id="instructor" name="instructor" multiple="multiple" <?= _current_webinar() ?? "disabled='disabled'" ?>>
                                @if(count(_get_instructors(_current_provider()->id)) > 0)
                                    @foreach ( _get_instructors(_current_provider()->id) as $instructor)
                                    @if($instructor->status=="active") 
                                    <option value="{{$instructor->id}}" {{ $instructor_ids ? (in_array($instructor->id, $instructor_ids) ? "selected='selected'" : "") : "" }} ><?= $instructor->name; ?></option>
                                    @endif
                                    @endforeach
                                @else
            ...
            <div class="kt-portlet kt-portlet--mobile">
                <div class="kt-portlet__body">
                    <span><i class="fa fa-user-check" style="color:#2A7DE9;"></i> All visible instructors of this webinar must complete their profile before the webinar can be published. This includes name,
...
<script>
    ...
    $(`#removal-modal-submit`).click(function (event) {
        ...
        setTimeout(() => {
            $.ajax({
                url: '/webinar/management/instructor/list/filter',
                data: {
        ...
        var userTable = function() {
            var dataSource = {
                data: {
                    ...
                    source: {
                        read: {
                            url: '/webinar/management/instructor/list',
            ...
            var dataStructure = {
                    ...
                    },    
                {
                    ...
                    field: 'webinar_details',
                    title: 'Webinar Details',
                    template: function(row) {
                        var checked = "";
                        if (row.webinar_details === 1) {
                            checked = "checked";
                        }
                        return '\
                            <div class="kt-checkbox-list">\
                                <label class="kt-checkbox kt-checkbox--bold kt-checkbox--success">\
                                    <input '+(row.user_id==myRecordID ? "disabled" : "")+' type="checkbox" ' + checked + ' name="webinar_details_check" data-id="'+row.user_id+'" data-column="webinar_details" id="webinar_details_'+row.id+'" class ="permission" '+row.co_provider+' /> <b style="color:white;">.</b> \
                                    <span></span>\
                                </label>\
                            </div>\
                            ';
            ...
            datatable.on('click', `input[class='permission']`,  function(){
                ...
                $.ajax({
                    url: '/webinar/management/instructor/permission',
                    type: 'POST',
                    data: {
                        user_id: id,
                        column: column,
                ...
                submitHandler: function(form) {
                    ...
                    if($(`#instructor`).val()){
                        var submit_form = $.ajax({
                            url: '/webinar/management/instructor/action',
                            type: 'POST',
                            data: {
                            ...
                            success: function(response) {
                                if (response.status == 200) {
                                    ...
                                    setTimeout(() => {
                                        window.location = "/webinar/management/instructors";
                                    }, 1000);
                                } else {
