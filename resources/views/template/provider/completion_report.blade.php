	...
	<div class="container" class="margin-top:-50px">
		...
		<div class="row">
			...
			<table width="100%">
				...
				<tr>
					<td class="font-s " width="50%">
						Date Started: <?= date("F j, Y",strtotime($courses->session_start)) ?>
					</td>
					<td class="font-s " colspan="2" width="50%">
						Date Completed: <?= date("F j, Y",strtotime($courses->session_end)) ?>
					</td>
				</tr>
				<tr>
					<td colspan="3" class="font-s" >
				...
				<tr>
					...
					<td class="font-s " colspan="2" width="50%">
						Date Applied: <?= date("F j, Y",strtotime($courses->created_at)) ?>
					</td>
				</tr>
				<tr>
					<td colspan="3" class="font-s" style="border-bottom:none;">
						...
						<p>
							<?= $general_info->provider_name ?> has created the program course title <?= $courses->title ?> in the online platform FastCPD.com that was published since <?= date("F j, Y",strtotime($courses->published_at)) ?>. 
							For the month of <?= date("F",strtotime($completion_rep->created_at)) ?> of the year <?= date("Y",strtotime($completion_rep->created_at)) ?>, a total of <?= $completion_rep->participants ?> 
							enrolled in the online course containing ( <?= intval($total_minutes/60) ?> hours and <?= round(fmod($total_minutes,60)) ?> minutes or <?= round($total_minutes) ?> minutes ) of
						...
						<div style="margin-left:20px;">
							<ol>
								@foreach($section_details as $details)
									<li>
										<?=$details["section_name"]?> 
										- <?=$details["videos_count"]?> videos, <?=$details["quizzes_count"]?> quizzes, <?=$details["articles_count"]?> articles
									</li>
								@endforeach
							</ol>
						</div>
			...
			<div class="container" class="margin-top:-50px">
				...
				<div class="row">
					<table width="100%">
						...
						<tr>
							<td class="font-s" colspan="2">Date: <?= date("F 01, Y",strtotime($completion_rep->completion_date)) ?> - <?= date("F t, Y",strtotime($completion_rep->completion_date)) ?></td>
						...
						<tr>
							<td colspan="3" style="border-top:none;">Date & Time:</td>
							<td colspan="2" style="border-top:none;">Date & Time:</td>
						</tr>
					</table>
				</div>
			</div>
