...
{
    function provider_instructors(Request $request) : JsonResponse
    {
        ...
        if($instructors->count() > 0){
            $total_count = Instructor::select("id")->where([
                "provider_id" => $provider,
                "status" => "active",
                "deleted_at" => null,
            ])->get()->count();
            ...
            $instructors = array_map(function($inst){
                ...
                ->whereRaw("JSON_CONTAINS(instructor_id, '\"{$inst["user_id"]}\"')")->get()->count();
            ...
            return response()->json(["data"=>$instructors, "total" => $total_count]);
        }
    ...
    function provider_courses(Request $request) : JsonResponse
    {
        ...
        $total = Course::select("id")->where("published_at", "!=", NULL)->where(["prc_status" => "approved"])->whereIn("fast_cpd_status", ["published", "live", "ended"])->where("provider_id", $provider)->orderBy('published_at', 'desc')->get();
    ...
    function provider_webinars(Request $request) : JsonResponse
    {
        ...
        if($provider){
            ...
            )->whereIn("fast_cpd_status", ["published", "live", "ended"])->where(["deleted_at" => null, "provider_id"=>$provider])
            ...
            return response()->json(["data"=>$webinar_collection, "total"=>_provider_webinars($provider, true)], 200);
        }
